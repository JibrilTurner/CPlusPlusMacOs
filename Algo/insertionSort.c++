...
int main() {
    int arr[] = {88, 74, 41, 63, 8};
    int n = sizeof(arr) / sizeof(arr[0]);
        srand(time(NULL));

    // add 100 random elements to the array
    for(int i = 0; i < 100; i++) {
        int randomNum = rand() % 1000; // generate a random number between 0 and 999
        int index = rand() % n; // generate a random index in the array
        arr[index] += randomNum; // add the random number to the element at the random index
    }
